...
- **Unbounded (default):** Loop until the user stops the session
- **Bounded:** Loop exactly N times when the user specifies a max iteration count
...
The guard is a command that must ALWAYS pass — it protects existing functionality while the main metric is being optimized. Common guards: `pytest`, `mypy src/`, `flake8 src/`, `python -m py_compile`.
...
### Bounded Mode (with max iteration count)
